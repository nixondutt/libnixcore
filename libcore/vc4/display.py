...
class Display(object):
    ...
    def get_info(self):
        ...
        Returns:
            :class:`~libcore.vc4.display.DISPMANX_T`: display information
        """

        self.info = DISPMANX_MODEINFO_T()
        result = _bcm_host.vc_dispmanx_display_get_info(
            self.handle, byref(self.info))
        if result != 0:
            self.info = None
            raise RuntimeError(
                "Failed to get display({}) information.".format(
                    self.display_num))
        return self.info
    ...
    def open_window(self, dst, size, layer):
        ...
        Args:
            ...
            layer (int): layer
        Returns:
            :class:`~libcore.vc4.display.Window`: window
        """
...
class Window(object):
    ...
    def __init__(self, display, dst, size, layer):
        ...
        for i in range(self.num_of_resources):
            ...
            handle = _bcm_host.vc_dispmanx_resource_create(
                self.format, size[0], size[1], byref(
                    self.native_image_handle[i]))
            if handle == 0:
        ...
        dst_rect = VC_RECT_T()
        _bcm_host.vc_dispmanx_rect_set(
            byref(dst_rect), dst[0], dst[1], dst[2], dst[3])
    ...
    def blit(self, image):
        ...
        Args:
            image(bytes): RGB image with size which size is the same as window size
        """
